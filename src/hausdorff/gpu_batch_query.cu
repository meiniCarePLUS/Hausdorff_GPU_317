...
// Pinned host buffers + pre-allocated device buffers for per-iteration queries.
static const int  QUERY_BUF   = 128;
static double*    g_h_pts      = nullptr;  // pinned host input
static int*       g_h_nearest  = nullptr;  // pinned host output
...
void gpu_plain_init_B(const double* mesh_B_verts, int nB) {
    ...
    cudaMallocHost(&g_h_pts,     QUERY_BUF * 3 * sizeof(double));
    cudaMallocHost(&g_h_nearest, QUERY_BUF * sizeof(int));
...
void gpu_plain_free_B() {
    ...
    cudaFreeHost(g_h_pts);   g_h_pts    = nullptr;
    cudaFreeHost(g_h_nearest);g_h_nearest= nullptr;
    cudaFree(g_d_pts);       g_d_pts    = nullptr;
    cudaFree(g_d_nearest);   g_d_nearest= nullptr;
    cudaStreamDestroy(g_stream); g_stream = nullptr;
    g_nB = 0;
}

void gpu_plain_query(const double* pts, int n_pts, int* out_nearest) {
    if (n_pts <= QUERY_BUF) {
        // Use pinned buffers + async stream for minimal latency.
        memcpy(g_h_pts, pts, n_pts * 3 * sizeof(double));
        cudaMemcpyAsync(g_d_pts, g_h_pts, n_pts * 3 * sizeof(double),
                        cudaMemcpyHostToDevice, g_stream);
        int block = 32, grid = (n_pts + block - 1) / block;
        kernel_point_query<<<grid, block, 0, g_stream>>>(
            g_d_pts, n_pts, g_d_tris_B, g_lbvh.d_nodes, g_d_nearest);
        cudaMemcpyAsync(g_h_nearest, g_d_nearest, n_pts * sizeof(int),
                        cudaMemcpyDeviceToHost, g_stream);
        cudaStreamSynchronize(g_stream);
        memcpy(out_nearest, g_h_nearest, n_pts * sizeof(int));
    } else {
        double* d_pts; int* d_out;
        ...
        int block = 128, grid = (n_pts + block - 1) / block;
